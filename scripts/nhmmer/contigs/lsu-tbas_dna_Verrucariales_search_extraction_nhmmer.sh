...
#SBATCH --partition=scavenger # Partition to be used to run the script
            ...
            then
            	if [ "no" = "yes" ];
            		then
						...
						rm -r nhmmer/Verrucariales/${count}/sequences/lsu-tbas
						mkdir -p nhmmer/Verrucariales/${count}/nhmmer/lsu-tbas/dna
						mkdir -p nhmmer/Verrucariales/${count}/sequences/lsu-tbas/dna
						mkdir -p nhmmer/Verrucariales/${count}/sequences/lsu-tbas/dna/single_hits

						#Defining variables on the directories locations
						# mafft
						alignment=$(echo nhmmer/"Verrucariales"/"${count}"/alignment)
						# nhmmer
						nhmmer=$(echo nhmmer/"Verrucariales"/"${count}"/nhmmer/"lsu-tbas"/"dna")
						sequences=$(echo nhmmer/"Verrucariales"/"${count}"/sequences/"lsu-tbas"/"dna")
						single_hits=$(echo nhmmer/"Verrucariales"/"${count}"/sequences/"lsu-tbas"/"dna"/single_hits)
					else
						# nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r nhmmer/Verrucariales/${count}/sequences/lsu-tbas
				...
				if [ "no" = "yes" ];
            		then
		                ...
		                # nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r analyses/nhmmer/Verrucariales/${count}/sequences/lsu-tbas
						mkdir -p analyses/nhmmer/Verrucariales/${count}/nhmmer/lsu-tbas/dna
						mkdir -p analyses/nhmmer/Verrucariales/${count}/sequences/lsu-tbas/dna
						mkdir -p analyses/nhmmer/Verrucariales/${count}/sequences/lsu-tbas/dna/single_hits

						#Defining variables on the directories locations
						# mafft
						alignment=$(echo "analyses"/nhmmer/"Verrucariales"/"${count}"/alignment)
						# nhmmer
						nhmmer=$(echo "analyses"/nhmmer/"Verrucariales"/"${count}"/nhmmer/"lsu-tbas"/"dna")
						sequences=$(echo "analyses"/nhmmer/"Verrucariales"/"${count}"/sequences/"lsu-tbas"/"dna")
						single_hits=$(echo "analyses"/nhmmer/"Verrucariales"/"${count}"/sequences/"lsu-tbas"/"dna"/single_hits)
					else
						# nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r analyses/nhmmer/Verrucariales/${count}/sequences/lsu-tbas					
		...
		if [ "" = "+" ]; 
			then
				nhmmer --dna --cpu 1 --watson --incE 1.0e-30 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/reads/Verrucariales//${count}_Verrucariales_2.fq
		elif [ "+-" = "-" ];
			then
				nhmmer --dna --cpu 1 --crick --incE 1.0e-30 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/reads/Verrucariales//${count}_Verrucariales_2.fq
			else
				nhmmer --dna --cpu 1 --incE 1.0e-30 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/reads/Verrucariales//${count}_Verrucariales_2.fq
		fi
		...
		# USING SEQKIT
			...
			if [ "${number_hits}" -eq 0 ]; then
				...
				if [ "0" -gt 0 ]; then
		        	...
		        	if [ "analyses" = "." ]; then
		        		# Creating directories
						mkdir -p nhmmer/Verrucariales/${count}/sequences/lsu-tbas/dna/best_hits
						#Defining variables on the directories locations
						best_hits=$(echo nhmmer/"Verrucariales"/"${count}"/sequences/"lsu-tbas"/"dna"/best_hits)
					else
		        		# Creating directories
						mkdir -p analyses/nhmmer/Verrucariales/${count}/sequences/lsu-tbas/dna/best_hits
						#Defining variables on the directories locations
						best_hits=$(echo "analyses"/nhmmer/"Verrucariales"/"${count}"/sequences/"lsu-tbas"/"dna"/best_hits)
					fi

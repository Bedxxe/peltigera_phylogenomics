...
#SBATCH --output=logs/nhmmer/contigs/its-lsu_Candelariales_nhmmer_Candelariales_%A_%a.out # Indicate a path where a file with the output information will be created. That directory must already exist
#SBATCH --error=logs/nhmmer/contigs/its-lsu_Candelariales_nhmmer_Candelariales_%A_%a.err # Indicate a path where a file with the error information will be created. That directory must already exist
#SBATCH --partition=scavenger # Partition to be used to run the script
            ...
            then
            	if [ "no" = "yes" ];
            		then
		            	...
		            	# mafft
						mkdir -p nhmmer/its-lsu_Candelariales/${count}/alignment 
		                # nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas
						mkdir -p nhmmer/its-lsu_Candelariales/${count}/nhmmer/lsu-tbas/dna
						mkdir -p nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas/dna
						mkdir -p nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas/dna/single_hits

						#Defining variables on the directories locations
						# mafft
						alignment=$(echo nhmmer/"its-lsu_Candelariales"/"${count}"/alignment)
						# nhmmer
						nhmmer=$(echo nhmmer/"its-lsu_Candelariales"/"${count}"/nhmmer/"lsu-tbas"/"dna")
						sequences=$(echo nhmmer/"its-lsu_Candelariales"/"${count}"/sequences/"lsu-tbas"/"dna")
						single_hits=$(echo nhmmer/"its-lsu_Candelariales"/"${count}"/sequences/"lsu-tbas"/"dna"/single_hits)
					else
						# nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas
				...
				if [ "no" = "yes" ];
            		then
            			# Creating directories
		            	# mafft
						mkdir -p analyses/nhmmer/its-lsu_Candelariales/${count}/alignment 
		                # nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r analyses/nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas
						mkdir -p analyses/nhmmer/its-lsu_Candelariales/${count}/nhmmer/lsu-tbas/dna
						mkdir -p analyses/nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas/dna
						mkdir -p analyses/nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas/dna/single_hits

						#Defining variables on the directories locations
						# mafft
						alignment=$(echo "analyses"/nhmmer/"its-lsu_Candelariales"/"${count}"/alignment)
						# nhmmer
						nhmmer=$(echo "analyses"/nhmmer/"its-lsu_Candelariales"/"${count}"/nhmmer/"lsu-tbas"/"dna")
						sequences=$(echo "analyses"/nhmmer/"its-lsu_Candelariales"/"${count}"/sequences/"lsu-tbas"/"dna")
						single_hits=$(echo "analyses"/nhmmer/"its-lsu_Candelariales"/"${count}"/sequences/"lsu-tbas"/"dna"/single_hits)
					else
						# nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r analyses/nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas					
		...
		if [ "" = "+" ]; 
			then
				nhmmer --dna --cpu 1 --watson --incE 1.0e-30 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/contigs/Candelariales/${count}_Candelariales.fasta
		elif [ "+-" = "-" ];
			then
				nhmmer --dna --cpu 1 --crick --incE 1.0e-30 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/contigs/Candelariales/${count}_Candelariales.fasta
			else
				nhmmer --dna --cpu 1 --incE 1.0e-30 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/contigs/Candelariales/${count}_Candelariales.fasta
		fi
		...
		# USING SEQKIT
			...
			if [ "${number_hits}" -eq 0 ]; then
				...
				if [ "0" -gt 0 ]; then
		        	...
		        	if [ "analyses" = "." ]; then
		        		# Creating directories
						mkdir -p nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas/dna/best_hits
						#Defining variables on the directories locations
						best_hits=$(echo nhmmer/"its-lsu_Candelariales"/"${count}"/sequences/"lsu-tbas"/"dna"/best_hits)
					else
		        		# Creating directories
						mkdir -p analyses/nhmmer/its-lsu_Candelariales/${count}/sequences/lsu-tbas/dna/best_hits
						#Defining variables on the directories locations
						best_hits=$(echo "analyses"/nhmmer/"its-lsu_Candelariales"/"${count}"/sequences/"lsu-tbas"/"dna"/best_hits)
					fi

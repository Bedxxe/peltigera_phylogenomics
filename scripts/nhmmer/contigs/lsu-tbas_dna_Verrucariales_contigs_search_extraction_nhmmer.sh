...
#SBATCH --partition=scavenger # Partition to be used to run the script
            ...
            then
            	if [ "no" = "yes" ];
            		then
						...
						rm -r nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas
						mkdir -p nhmmer/Verrucariales_contigs/${count}/nhmmer/lsu-tbas/dna
						mkdir -p nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas/dna
						mkdir -p nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas/dna/single_hits

						#Defining variables on the directories locations
						# mafft
						alignment=$(echo nhmmer/"Verrucariales_contigs"/"${count}"/alignment)
						# nhmmer
						nhmmer=$(echo nhmmer/"Verrucariales_contigs"/"${count}"/nhmmer/"lsu-tbas"/"dna")
						sequences=$(echo nhmmer/"Verrucariales_contigs"/"${count}"/sequences/"lsu-tbas"/"dna")
						single_hits=$(echo nhmmer/"Verrucariales_contigs"/"${count}"/sequences/"lsu-tbas"/"dna"/single_hits)
					else
						# nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas
				...
				if [ "no" = "yes" ];
            		then
		                ...
		                # nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r analyses/nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas
						mkdir -p analyses/nhmmer/Verrucariales_contigs/${count}/nhmmer/lsu-tbas/dna
						mkdir -p analyses/nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas/dna
						mkdir -p analyses/nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas/dna/single_hits

						#Defining variables on the directories locations
						# mafft
						alignment=$(echo "analyses"/nhmmer/"Verrucariales_contigs"/"${count}"/alignment)
						# nhmmer
						nhmmer=$(echo "analyses"/nhmmer/"Verrucariales_contigs"/"${count}"/nhmmer/"lsu-tbas"/"dna")
						sequences=$(echo "analyses"/nhmmer/"Verrucariales_contigs"/"${count}"/sequences/"lsu-tbas"/"dna")
						single_hits=$(echo "analyses"/nhmmer/"Verrucariales_contigs"/"${count}"/sequences/"lsu-tbas"/"dna"/single_hits)
					else
						# nhmmer
		                # REMOVING FORMER OUTPUTS 
						rm -r analyses/nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas					
		...
		if [ "" = "+" ]; 
			then
				nhmmer --dna --cpu 1 --watson --incE 1.0e-10 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/contigs/Verrucariales//${count}_Verrucariales.fasta
		elif [ "+-" = "-" ];
			then
				nhmmer --dna --cpu 1 --crick --incE 1.0e-10 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/contigs/Verrucariales//${count}_Verrucariales.fasta
			else
				nhmmer --dna --cpu 1 --incE 1.0e-10 				--tblout $nhmmer/${count}_lsu-tbas.tbl 				-A $nhmmer/${count}_lsu-tbas.multialignment 				-o $nhmmer/${count}_lsu-tbas_nhmmer.output 				${nhmmer_profile} problematic_otus/contigs/Verrucariales//${count}_Verrucariales.fasta
		fi
		...
		# USING SEQKIT
			...
			if [ "${number_hits}" -eq 0 ]; then
				...
				do 
					...
					seqkit subseq problematic_otus/contigs/Verrucariales//${count}_Verrucariales.fasta -r $beg:$endo --chr "$hedr" -o ${single_hits}/"${count}"_"lsu-tbas"_"$node"_length_"$seq_lenght".fasta;
				...
				if [ "0" -gt 0 ]; then
		        	...
		        	if [ "analyses" = "." ]; then
		        		# Creating directories
						mkdir -p nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas/dna/best_hits
						#Defining variables on the directories locations
						best_hits=$(echo nhmmer/"Verrucariales_contigs"/"${count}"/sequences/"lsu-tbas"/"dna"/best_hits)
					else
		        		# Creating directories
						mkdir -p analyses/nhmmer/Verrucariales_contigs/${count}/sequences/lsu-tbas/dna/best_hits
						#Defining variables on the directories locations
						best_hits=$(echo "analyses"/nhmmer/"Verrucariales_contigs"/"${count}"/sequences/"lsu-tbas"/"dna"/best_hits)
					fi
					...
					do 
						...
						seqkit subseq problematic_otus/contigs/Verrucariales//${count}_Verrucariales.fasta -r $beg:$endo --chr "$hedr" -o ${best_hits}/"${count}"_"lsu-tbas"_"$node"_length_"$seq_lenght".fasta;
		...
		#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			if [ "${number_hits}" -eq 0 ]; then
				...
				do 
					...
					/hpc/group/bio1/diego/programs/faSomeRecords/faSomeRecords.py -f problematic_otus/contigs/Verrucariales//${count}_Verrucariales.fasta -r "${hedr}" -o ${single_hits}/"${count}"_"lsu-tbas"_"$node"_length_"$seq_lenght".fasta;
					old_label=$(grep '>' ${single_hits}/"${count}"_"lsu-tbas"_"$node"_length_"$seq_lenght".fasta | cut -d' ' -f1);
					new_label=$(echo '>'"${count}"_"lsu-tbas"_"$node"_"${seq_lenght}");
		        	sed -i "s/${old_label} .*/${new_label}/g" ${single_hits}/"${count}"_"lsu-tbas"_"$node"_length_"$seq_lenght".fasta;
					# Concatenating the sequences

...
# This script is to assemble reads using spades
...
kmers=${11} # Give the number the lenght of the k-mers that you want to use. Three options: 
#default    21,33,55,77
#deep       21,29,39,49,59,79,97,105,117
#custom     Give your own values   
metagenome=${12} # Indicate the program if you are trying to assemble a metagenomic sample(s) yes/no
plasmid=${13} # Indicate the program if you want to obtain the plasmid seuqences with spades
careful=${14} # indicate if the program mudt be run using the --careful flag. This flag does not work with the rna, metagenome, and plasmid options
keep_files=${15} # Indicate the program if you want to keep the assembly output files from all the k-mer runs in spades yes/no
rna_seq=${16} # Indicate the program if you are trying to assemble a RNA_seq sample(s)
hybrid=${17} # Please specify if you want to use a long-read set of data for the assembly. Three oprions: nanopore/pacbio/no
hybrid_path=${18} # Path to the long-reads fasta file 
hybrid_suff=${19} # Indicate the suffix of the long-reads files
slurm=${20} # Indicate the program if you would like to run the output script in a SLURM based cluster: 
if [ ${slurm} = "yes" ];
    then
        ...
        w_execute=${26} # Tell the script if you want to execute it on the cluster right after producing the final script. yes/no
...
if [ ${kmers} = "default" ]; then
    echo -e 'The kmers to use are:'"\t""\t""\t"'21,33,55,77'
elif [ ${kmers} = "deep" ]; then
    echo -e 'The kmers to use are:'"\t""\t""\t"'21,29,39,49,59,79,97,105,117'
else
    echo -e 'The kmers to use are:'"\t""\t""\t""${kmers}"
fi
if [ ${metagenome} = "yes" ];
    then
...
if [ ${hybrid} = "nanopore" ]; then
    echo -e 'Will use the nanopore reads located at:'"\t""\t""\t""${hybrid_path}"
elif [ ${hybrid} = "pacbio" ]; then
    echo -e 'Will use the nanopore reads located at:'"\t""\t""\t""${hybrid_path}"
fi
####
if [ ${slurm} = "yes" ]; 
...
# kmers
if [ ${kmers} = "default" ]; then
    kmers=$(echo '21,33,55,77')
elif [ ${kmers} = "deep" ]; then
    kmers=$(echo '21,29,39,49,59,79,97,105,117')
else
    kmers=$(echo "${kmers}")
fi
...
if [ ${hybrid} = "no" ]; then
    ...
    hybrid_path=$(echo ${hybrid_path}'/${count}'${hybrid_suff})
fi
    ...
    then 
       ...
       # MAIN SLUMRM SCRIPT
        ...
        module load SPAdes/3.15.4-rhel8
            ...
            -k ${kmers} -m ${total_ram} \
            ${metagenome} ${careful} ${rna_seq} ${hybrid} ${hybrid_path}
            ...
            if [ ${plasmid} = "yes" ];
                then
                    spades.py -1 ${reads_dir}/${sign}{count}/${sign}{count}${forw_suff} \
                    -2 ${reads_dir}/${sign}{count}${rev_suff} \
                    -o ${sign}{plasmids} -t ${cores} \
                    -k ${kmers}  -m ${total_ram} --meta --plasmid
                else

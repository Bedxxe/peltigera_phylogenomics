...
sample_dir=${7} # Location of the single files to classify
sample_suff=${8} # Suffix of the files to classift
...
if [ ${slurm} = "yes" ]; 
    then
        ...
        w_execute=${22} # Tell the script if you want to execute it on the cluster right after producing the final script
...
#SBATCH --partition=${par} # Partition to be used to run the script
        ...
        if [ "${out_dir}" = "." ]; 
            then
                # sequences
                mkdir -p taxonomy/kraken/${pref}/sequences
                # kraken
                mkdir -p taxonomy/kraken/${pref}/kraken/krakens
                mkdir -p taxonomy/kraken/${pref}/kraken/reports
                # bracken
                mkdir -p taxonomy/kraken/${pref}/bracken/brackens
                mkdir -p taxonomy/kraken/${pref}/bracken/reports

                #Defining variables on the directories locations
                # sequences
                sequences=${sign}(echo taxonomy/kraken/${pref}/sequences)
                # kraken
                krakens=${sign}(echo taxonomy/kraken/${pref}/kraken/krakens)
                k_reports=${sign}(echo taxonomy/kraken/${pref}/kraken/reports)
                # bracken
                brackens=${sign}(echo taxonomy/kraken/${pref}/bracken/brackens)
                b_reports=${sign}(echo taxonomy/kraken/${pref}/bracken/reports)
            else
                # sequences
                mkdir -p ${out_dir}/taxonomy/kraken/${pref}/sequences
                # kraken
                mkdir -p ${out_dir}/taxonomy/kraken/${pref}/kraken/krakens
                mkdir -p ${out_dir}/taxonomy/kraken/${pref}/kraken/reports
                # bracken
                mkdir -p ${out_dir}/taxonomy/kraken/${pref}/bracken/brackens
                mkdir -p ${out_dir}/taxonomy/kraken/${pref}/bracken/reports
                
                #Defining variables on the directories locations
                # sequences
                sequences=${sign}(echo "${out_dir}"/taxonomy/kraken/${pref}/sequences)
                # kraken
                krakens=${sign}(echo "${out_dir}"/taxonomy/kraken/${pref}/kraken/krakens)
                k_reports=${sign}(echo "${out_dir}"/taxonomy/kraken/${pref}/kraken/reports)
                # bracken
                brackens=${sign}(echo "${out_dir}"/taxonomy/kraken/${pref}/bracken/brackens)
                b_reports=${sign}(echo "${out_dir}"/taxonomy/kraken/${pref}/bracken/reports)
        fi
        ...
        -w ${sign}{b_reports}/${sign}{count}_bracken.report -r 150 -t 5
...
#!/bin/bash
        ...
        if [ "${out_dir}" = "." ]; 
            then
                # logs
                mkdir -p taxonomy/${pref}/logs
                # sequences
                mkdir -p taxonomy/${pref}/sequences
                # kraken
                mkdir -p taxonomy/kraken/${pref}/krakens
                mkdir -p taxonomy/kraken/${pref}/reports
                # bracken
                mkdir -p taxonomy/${pref}/bracken/brackens
                mkdir -p taxonomy/${pref}/bracken/reports

                #Defining variables on the directories locations
                # sequences
                sequences=${sign}(echo "${out_dir}"/taxonomy/${pref}/sequences)
                # kraken
                krakens=${sign}(echo "${out_dir}"/taxonomy/kraken/${pref}/krakens)
                k_reports=${sign}(echo "${out_dir}"/taxonomy/kraken/${pref}/reports
                # bracken
                brackens=${sign}(echo "${out_dir}"/taxonomy/${pref}/bracken/brackens)
                b_reports=${sign}(echo "${out_dir}"/taxonomy/${pref}/bracken/reports)
            else

                # logs
                mkdir -p ${out_dir}/taxonomy/${pref}/logs
                # sequences
                mkdir -p ${out_dir}/taxonomy/${pref}/sequences
                # kraken
                mkdir -p ${out_dir}/taxonomy/kraken/${pref}/krakens
                mkdir -p ${out_dir}/taxonomy/kraken/${pref}/reports
                # bracken
                mkdir -p ${out_dir}/taxonomy/${pref}/bracken/brackens
                mkdir -p ${out_dir}/taxonomy/${pref}/bracken/reports
...
EOF
        ...
        if [ "${out_dir}" = "." ]; 
                then
                    mv ${pref}_taxonomy_kraken_bracken.sh taxonomy/${pref}/logs/${pref}_taxonomy_kraken_bracken.sh
                    sh taxonomy/${pref}/logs/${pref}_fastp_trimming.sh 
                else
                    mv ${pref}_taxonomy_kraken_bracken.sh ${out_dir}/taxonomy/${pref}/logs/${pref}_taxonomy_kraken_bracken.sh
                    sh ${out_dir}/taxonomy/${pref}/logs/${pref}_taxonomy_kraken_bracken.sh
        fi 
fi

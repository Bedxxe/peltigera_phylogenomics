...
#	faSomeRecords	https://github.com/santiagosnchez/faSomeRecords	wget https://raw.githubusercontent.com/santiagosnchez/faSomeRecords/master/faSomeRecords.py
...
# IMPORTANT that this value cannont be less than 1.0e-300
inclusion_evalue=${8} # Tell the program the threshold of e-value for extracting the sequences from the output table. This is a different value than the one nhmmer uses for the search. 
# IMPORTANT use exponential notation to declare this variable e.j. 1.0e-15
hmm_profile=${9} # Indicate if the user already have a nhmmer profile file that has been created from an alignment. GIVE THE FULL PATH
mafft=${10} # Tell the program if you want to align a set of sequences to make the HMMER profile. yes/no
fasta=${11} # Location of the fasta file to be aligned if this was specified
user_alignment=${12} # Location of the alignment that the user is providing. If the user do not provide an alignment, write "-"
gene=${13} # The gen to analyze
alphabet=${14} # The type of sequence that will be searched:
#  dna   : input alignment is DNA sequence data
#  rna   : input alignment is RNA sequence data
strand=${15} # Indicate in which strand to look for information:
#	-	:Just look on the "-" strand of DNA. Former named Crick
#	+	:Just look on the "+" strand of DNA. Former named Watson
#	+-	:Look on both strands
best_hits=${16} # Indicate the program if you want to obtain just a specific number of hits regadless of the threshold evaluation. If not, write 0
query_type=${17} # Indicate the program if your query is an assembly (1) or a file with reads (2) 1/2
query=${18} # Path to the assembly/fasta that you want to use as query
query_suff=${19} # The suffix of the query sequence(s) e.g. *.fasta
up_stream=${20} # Give the number of extra upstream nucleotides to get extracted. If you do not want this option, write "0"
down_stream=${21} # Give the number of extra downstream nucleotides to get extracted. If you do not want this option, write "0"
slurm=${22} # Indicate the program if you would like to run the output script in a SLURM based cluster: yes/no
if [ ${slurm} = "yes" ]; 
    then
        ram=${23} # Ammount of Gb to use to each thread
        log_files=${24} # The name of both, the output and the error files that SLURM creates
        log_dir=${25} # Directory where to place a folder to put the logs
        script_dir=${26} # Directory where to place the script once it has been created
        par=${27} # The name of the partition where the user wants the job to be run
        w_execute=${28} # Tell the script if you want to execute it on the cluster right after producing the final script
        n_samp=$(cat ${samp} | sort | uniq | wc -l) # The number of iterations in the array of SLURM
        total_ram=$(echo $(("${cores}" * "${ram}")))
    else
        max_jobs=${22} # The maximun number of jobs that the user wants to run in the local computer
fi
...
####
echo -e 'Threshold of e-value for nhmmer to include outputs:'"\t""\t""\t""${threshold}"
echo -e 'Threshold of e-value for extracting the outputs into fasta files:'"\t""\t""\t""${inclusion_evalue}"
if [ ${hmm_profile} != "no" ];
    then
...
if [ ${query_type} -eq 1 ];
    then
        echo -e 'User is using an assembly as a query'
    else
        echo -e 'User is using a big file of reads. Not an assembly!'
fi
	...
	# MAIN SLUMRM SCRIPT
		...
		# USING SEQKIT
			...
			if [ "${sign}{number_hits}" -eq 0 ]; then
				...
				echo -e "#Contig_name"'\t'"#From"'\t'"#To"'\t'"#Strand"'\t'"#E-value"'\t'"#Lenght"'\t'"#Original_start"'\t'"#Original_ending"'\t'"#Threshold"'\t'"#Notes" > ${sign}single_hits/${sign}{count}_${gene}_hits_information.tsv

				# Reading information to the created file
				thr=${sign}(echo ${inclusion_evalue} )
				...
				if [ "${sign}seq_end" -gt "${sign}seq_beg" ] # We will rearrenge the output
					then
						...
						if [ "${sign}x1" -eq 1 ]; then
							echo -e ${sign}hedr'\t'${sign}adj_seq_beg'\t'${sign}adj_seq_end'\t'${sign}orientation_strand'\t'${sign}evl'\t'${sign}seq_lenght'\t'${sign}seq_beg'\t'${sign}seq_end'\t'"${sign}thr"'\t'"${sign}note" >> ${sign}single_hits/${sign}{count}_${gene}_hits_information.tsv
						fi
					else
						contig_edge_beg=${sign}(echo ${sign}(("${sign}seq_beg" + "${up_stream}")))
						if [ "${sign}contig_edge_beg" -gt "0" ]; then				
						...
						if [ "${sign}x1" -eq 1 ]; then
							echo -e ${sign}hedr'\t'${sign}adj_seq_end'\t'${sign}adj_seq_beg'\t'${sign}orientation_strand'\t'${sign}evl'\t'${sign}seq_lenght'\t'${sign}seq_end'\t'${sign}seq_beg'\t'"${sign}thr"'\t'"${sign}note" >> ${sign}single_hits/${sign}{count}_${gene}_hits_information.tsv
						fi
				fi;
				done
				...
				do 
					...
					seqkit subseq ${query}/${sign}{count}${query_suff} -r ${sign}beg:${sign}endo --chr "${sign}hedr" -o ${sign}{single_hits}/"${sign}{count}"_"${gene}"_"${sign}node"_length_"${sign}seq_lenght".fasta;
					old_label=${sign}(grep '>' ${sign}{single_hits}/"${sign}{count}"_"${gene}"_"${sign}node"_length_"${sign}seq_lenght".fasta | cut -d' ' -f1);
					new_label=${sign}(echo '>'"${sign}{count}"_"${gene}"_"${sign}node"_"${sign}{seq_lenght}");
		        	sed -i "s/${sign}{old_label} .*/${sign}{new_label}/g" ${sign}{single_hits}/"${sign}{count}"_"${gene}"_"${sign}node"_length_"${sign}seq_lenght".fasta;
					# Concatenating the sequences
					cat ${sign}{single_hits}/"${sign}{count}"_"${gene}"_"${sign}node"_length_"${sign}seq_lenght".fasta >> ${sign}{single_hits}/${sign}{count}_${gene}_all_concatenated.fasta;
		...
		#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
			...
			if [ "${sign}{number_hits}" -eq 0 ]; then
				...
				#Creating a file where to held the information
				touch ${sign}single_hits/${sign}{count}_${gene}_hits_information.tsv 
				echo -e "#Contig_name"'\t'"#Strand"'\t'"#E-value"'\t'"#Lenght"'\t'"#Threshold" > ${sign}single_hits/${sign}{count}_${gene}_hits_information.tsv

				# Reading information to the created file
				thr=${sign}(echo ${inclusion_evalue} )
				alignment_query=${sign}(cat ${sign}nhmmer/${sign}{count}_${gene}.tbl | sed -n 3p | awk -F ' ' '{print${sign}3}')
				cat ${sign}nhmmer/${sign}{count}_${gene}.tbl | grep "${sign}{alignment_query}" | grep -v 'Query file' | grep -v 'Option settings'| while read line; do 
					hedr=${sign}(echo ${sign}line | cut -d' ' -f1);
					orientation_strand=${sign}(echo ${sign}line | cut -d' ' -f12);
					seq_lenght=${sign}(echo ${sign}line | cut -d' ' -f11);
					evl=${sign}(echo ${sign}line | cut -d' ' -f13 );
					x1=${sign}(echo ${sign}evl ${sign}thr | awk '{if (${sign}1 <= ${sign}2) print "1"; else print "0"}');

					if [ "${sign}x1" -eq 1 ]; then
						echo -e ${sign}hedr'\t'${sign}orientation_strand'\t'${sign}evl'\t'${sign}seq_lenght'\t'"${sign}thr"'\t' >> ${sign}single_hits/${sign}{count}_${gene}_hits_information.tsv
					fi
				done		
				...
				do 
					hedr=${sign}(echo ${sign}line | cut -d' ' -f1);
					node=${sign}(echo ${sign}hedr | cut -d'_' -f1,2);
					seq_lenght=${sign}(echo ${sign}line | cut -d' ' -f4);
					/hpc/group/bio1/diego/programs/faSomeRecords/faSomeRecords.py -f ${query}/${sign}{count}${query_suff} -r "${sign}{hedr}" -o ${sign}{single_hits}/"${sign}{count}"_"${gene}"_"${sign}node"_length_"${sign}seq_lenght".fasta;
					old_label=${sign}(grep '>' ${sign}{single_hits}/"${sign}{count}"_"${gene}"_"${sign}node"_length_"${sign}seq_lenght".fasta | cut -d' ' -f1);
					new_label=${sign}(echo '>'"${sign}{count}"_"${gene}"_"${sign}node"_"${sign}{seq_lenght}");
		        	sed -i "s/${sign}{old_label} .*/${sign}{new_label}/g" ${sign}{single_hits}/"${sign}{count}"_"${gene}"_"${sign}node"_length_"${sign}seq_lenght".fasta;
					# Concatenating the sequences

...
# 9) Location of kraken2 database inside the cluster:            /hpc/group/bio1/diego/programs/kraken2/09092023
...
get_unclassified=${12} # Tell the interfase if you want the unclassified reads in a separate file: yes/no
get_nonbacterial=${13} # Tell the interfase if you want the reads not classified as Bacteria in a separate file: yes/no
...
slurm=${17} # Indicate the program if you would like to run the output script in a SLURM based cluster: yes/no
if [ ${slurm} = "yes" ]; 
    then
...
#SBATCH --partition=${par} # Partition to be used to run the script
        ...
        if [ "${first_otu}" = "-" ]; 
            then
                ...
                id_1_otu=${sign}(grep -m1 "${sign}(printf '\t')${first_otu}${sign}(printf '\t')" ${names_path} | grep -o '[0-9]*')
                # Writting into .tsv file
        ...
        if [ "${second_otu}" = "-" ]; 
            then
                ...
                id_2_otu=${sign}(grep -m1 "${sign}(printf '\t')${second_otu}${sign}(printf '\t')" ${names_path} | grep -o '[0-9]*')
                # Writting into .tsv file
        ...
        if [ "${third_otu}" = "-" ]; 
            then
                ...
                id_3_otu=${sign}(grep -m1 "${sign}(printf '\t')${third_otu}${sign}(printf '\t')" ${names_path} | grep -o '[0-9]*')
                # Writting into .tsv file

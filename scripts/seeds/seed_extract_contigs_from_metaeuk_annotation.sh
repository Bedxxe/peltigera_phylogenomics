...
#Rhizobium	379	227290;379
#Agrobacterium	357	227290;357
kraken_trim=${8} # Tell the script if you want to trim the extracted files to use them with kraken: yes/no
slurm=${9} # Indicate the program if you would like to run the output script in a SLURM based cluster: yes/no
if [ ${slurm} = "yes" ];
    then
    	cores=${10} # The numbere of cores to use.
        ram=${11} # Ammount of Gb to use to each thread
        log_files=${12} # The name of both, the output and the error files that SLURM creates
        log_dir=${13} # Directory where to place a folder to put the logs
        script_dir=${14} # Directory where to place the script once it has been created
        par=${15} # The name of the partition where the user wants the job to be run
        w_execute=${16} # Tell the script if you want to execute it on the cluster right after producing the final script
        n_samp=$(cat ${sample} | sort | uniq | wc -l) # The number of iterations in the array of SLURM
        total_ram=$(echo $(("${cores}" * "${ram}")))
    else
        max_jobs=${10} # The maximun number of jobs that the user wants to run in the local computer
fi
       ...
       # MAIN SLUMRM SCRIPT
		...
		if [ "${out_dir}" = "." ]; 
		    then
		        ...
		        mkdir -p contig_sorting/${pref}/temp/${sign}{count}
		        ...
		        temp=${sign}(echo contig_sorting/"${pref}"/temp/${sign}{count})
		        ...
		        mkdir -p ${out_dir}/contig_sorting/${pref}/temp/${sign}{count}
		        ...
		        temp=${sign}(echo "${out_dir}"/contig_sorting/"${pref}"/temp/${sign}{count})
        ...
        cat ${info_file} | while read line; do
        	...
        	grep "${sign}{decoy}" ${metaeuk_path}/${sign}{count}/${sign}{count}_tax_per_contig.tsv | awk -F ' ' '{print${sign}1}' > ${sign}{documents}/${sign}{taxa}_contigs.txt

			# GETTING THE DESIRED CONTIGS
			# Creating an empty file to store the information
			touch ${sign}{sorted}/${sign}{count}_${sign}{taxa}.fna
			# Getting the contigs from the list
			cat ${sign}{documents}/${sign}{taxa}_contigs.txt | while read line; do
			    contig=${sign}(echo ${sign}{line});
			    /hpc/group/bio1/diego/programs/faSomeRecords/faSomeRecords.py -f ${contigs_path}/${sign}{count}${contigs_suff} -r "${sign}{contig}" -o ${sign}{temp}/"${sign}{count}"_${sign}{contig}.fasta;
			    cat ${sign}{temp}/"${sign}{count}"_${sign}{contig}.fasta >> ${sign}{sorted}/${sign}{count}_${sign}{taxa}.fna
			    rm ${sign}{temp}/"${sign}{count}"_${sign}{contig}.fasta
			done
